...
def test_gameroot(gameroot: GameRoot):
    """Check that the gameroot exists properly."""
    assert isinstance(gameroot, GameRoot)


def test_get_character_codes(gameroot: GameRoot):
...
def test_get_object_tree(gameroot: GameRoot):
...
def test_get_anatomies(gameroot: GameRoot):
    """Test ability to retrieve anatomies from the game files."""
    anatomies = gameroot.get_anatomies()
    assert "Humanoid" in anatomies
    assert len(anatomies) > 50


def test_get_colors(gameroot: GameRoot):
    """Test ability to retrieve color codes from the game files."""
...
def test_get_genders(gameroot: GameRoot):
    """Test ability to retrieve genders from the game files."""
...
def test_gamever(gameroot: GameRoot):

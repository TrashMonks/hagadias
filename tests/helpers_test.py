...
def test_parse_qud_colors():
    ...
    assert parse_qud_colors("{{r|La}} {{r-R-R-W-W-w-w sequence|Jeunesse}}") == [
        ("La", "r"),
        (" ", None),
        ("Jeunesse", "r-R-R-W-W-w-w sequence"),
    ]
...
def test_strip_newstyle_qud_colors():
    ...
    assert (
        strip_newstyle_qud_colors("{{r|La}} {{r-R-R-W-W-w-w sequence|Jeunesse}}") == "La Jeunesse"
    )
    assert strip_newstyle_qud_colors("{{K|{{crysteel|crysteel}} mace}}") == "crysteel mace"
    assert (
        strip_newstyle_qud_colors("{{O|persistent {{G-W-o sequence|papaya}}}}")
        == "persistent papaya"
    )
